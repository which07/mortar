 ...
 * -param OUTPUT_PATH Output path for script data (e.g. s3n://my-output-bucket/<%= pigscript_name -%>)
 */

/**
 * User-Defined Functions (UDFs)
 */
REGISTER '../udfs/python/<%= pigscript_name -%>.py' using streaming_python as <%= pigscript_name -%>;
...
with_udf_output = FOREACH filtered 
                 GENERATE field0..field2, 
                          <%= pigscript_name -%>.example_udf(field0) AS example_udf_field;
